 ...
 * 
 * @WebMvcTest(ItemController.class) public class ItemControllerTest {
 * 
 * @Autowired private MockMvc mockMvc;
 * 
 * @MockBean private ItemBusinessService businessService;
 * 
 * @Test public void dummyItem_basic() throws Exception {
 * 
 ...
 * .andReturn(); //JSONAssert.assertEquals(expected,
 * result.getResponse().getContentAsString(), false);
 * 
 * }
 * 
 * @Test public void itemFromBusinessService_basic() throws Exception {
 * when(businessService.retreiveHardcodedItem()).thenReturn( new
 * Item(2,"Item2",10,10));
 * 
 ...
 * }
 * 
 * @Test public void retrieveAllItems_basic() throws Exception {
 * when(businessService.retrieveAllItems()).thenReturn( Arrays.asList(new
 * Item(2,"Item2",10,10), new Item(3,"Item3",20,20)) );
 * 
 * RequestBuilder request = MockMvcRequestBuilders
 * .get("/all-items-from-database") .accept(MediaType.APPLICATION_JSON);
 * 
 * MvcResult result = mockMvc.perform(request) .andExpect(status().isOk())
 * .andExpect(content().
 * json("[{id:3,name:Item3,price:20}, {id:2,name:Item2,price:10}]"))
 * .andReturn(); //JSONAssert.assertEquals(expected,
 * result.getResponse().getContentAsString(), false);
 * 
 * }
 * 
 * @Test public void retrieveAllItems_noitems() throws Exception {
